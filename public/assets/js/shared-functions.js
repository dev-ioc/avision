...
function initFileValidation(
  ...
  if (maxFileSize) {
    ...
    const phpMaxFileSize = document.querySelector(
      'meta[name="php-upload-max-filesize"]',
    )?.content;
    const phpPostMaxSize = document.querySelector(
      'meta[name="php-post-max-size"]',
    )?.content;
    if (phpMaxFileSize && phpPostMaxSize) {
      maxSize = Math.min(
        parsePhpSize(phpMaxFileSize),
        parsePhpSize(phpPostMaxSize),
      );
    } else {
...
function initPasswordToggle(passwordId, toggleId, iconId = null) {
  ...
  togglePassword.addEventListener("click", function (e) {
    ...
    togglePassword.title =
      type === "text" ? "Masquer le mot de passe" : "Afficher le mot de passe";
  });
}
    ...
    .then((data) => {
      if (data.sites && Array.isArray(data.sites)) {
        ...
        if (
          currentSiteId &&
          currentSiteName &&
          !data.sites.some((site) => site.id == currentSiteId)
        ) {
          const option = document.createElement("option");
...
function updateSelectedContract(
  ...
  function setContractOptions(items) {
    ...
    defaultOpt.value = "";
    defaultOpt.textContent = "Sélectionnez un contrat";
    contractSelect.appendChild(defaultOpt);
    ...
    if (contractSelect.tomselect) {
      ...
      if (currentContractId) {
        ...
        contractSelect.tomselect.setValue("", true);
      }
    } else if (currentContractId) {
      contractSelect.value = String(currentContractId);
    }
  }
 ...
 */
function updateTypeRequiresTravel(
  typeSelectId,
  typeRequiresTravelSelectId,
  typeRequiresTravelHiddenName,
) {
  const typeSelect = document.getElementById(typeSelectId);
  const typeRequiresTravelSelect = document.getElementById(
    typeRequiresTravelSelectId,
  );
  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      const requiresTravel =
        data.requires_travel == 1 || data.requires_travel === "1";
      const value = requiresTravel ? "1" : "0";
      typeRequiresTravelSelect.value = value;
      console.log(
        "Valeur du déplacement mise à jour:",
        value,
        "(requires_travel:",
        data.requires_travel,
        ")",
      );
    })
    .catch((error) => {
      console.error(
        "Erreur lors de la récupération des informations du type:",
        error,
      );
    });
}
...
function loadClientLocations(
  clientId,
  locationsGridSelector = ".locations-grid",
  callback = null,
) {
  ...
  fetch(`${baseUrl}user/get_client_locations?client_id=${clientId}`)
    ...
    .catch((error) => {
      ...
      locationsGrid.innerHTML =
        '<div class="alert alert-danger">Erreur lors du chargement des localisations.</div>';
    });
}
...
function buildLocationsInterface(data, container, existingLocations = null) {
  ...
  data.sites.forEach((site, siteIndex) => {
    ...
    if (site.rooms && site.rooms.length > 0) {
      ...
      siteBody.innerHTML =
        '<p class="text-muted ms-4">Aucune salle disponible pour ce site.</p>';
    }
  ...
  siteCheckboxes.forEach((siteCheckbox) => {
    siteCheckbox.addEventListener("change", function () {
      ...
      const roomCheckboxes = document.querySelectorAll(
        `.room-checkbox[data-site-id="${siteId}"]`,
      );
...
function toggleUserSections(typeSelectId, sections = {}) {
  ...
  if (sections.coefficientSection) {
    const coefficientSection = document.getElementById(
      sections.coefficientSection,
    );
    if (coefficientSection) {
      coefficientSection.style.display =
        userGroup === "Staff" ? "block" : "none";
    }
  }
  ...
  if (sections.clientSection) {
    ...
    const locationsContainer = document.getElementById(
      sections.locationsContainer || "locations-container",
    );
    const clientSelect = document.getElementById(
      sections.clientSelectId || "client_id",
    );
...
function loadPermissionsSimple(userType, permissionsSectionId) {
  ...
  // Vérifier que BASE_URL est défini
  const baseUrl =
    BASE_URL || window.BASE_URL || window.AppConfig?.BASE_URL || "";
  if (!baseUrl) {
    console.error("BASE_URL n'est pas défini");
    permissionsSection.innerHTML =
      '<p class="text-danger">Erreur de configuration: BASE_URL non défini.</p>';
    return;
  }

  // Afficher un indicateur de chargement
  permissionsSection.innerHTML =
    '<div class="text-center"><i class="fas fa-spinner fa-spin"></i> Chargement des permissions...</div>';
  permissionsSection.style.display = "block";

  // Récupérer l'ID de l'utilisateur depuis l'URL si on est en mode édition
  const urlParts = window.location.pathname.split("/");
  const userId =
    urlParts.includes("edit") && urlParts[urlParts.length - 1]
  ...
  if (!csrfToken) {
    ...
    permissionsSection.innerHTML =
      '<p class="text-danger">Erreur de configuration: token CSRF manquant. Veuillez recharger la page.</p>';
    return;
  }
  ...
  fetch(baseUrl + "user/load_permissions", {
    ...
    .then((response) => {
      ...
      if (!contentType || !contentType.includes("application/json")) {
        ...
        return response.text().then((text) => {
          ...
          throw new Error(
            "La réponse du serveur n'est pas au format JSON. Votre session a peut-être expiré.",
          );
        });
      }
    ...
    .then((data) => {
      if (data && data.html) {
        ...
        permissionsSection.innerHTML =
          '<p class="text-danger">' +
          (data.error || "Erreur lors du chargement des permissions.") +
          "</p>";
      } else {
        permissionsSection.innerHTML =
          '<p class="text-muted">Aucune permission disponible pour ce type d\'utilisateur.</p>';
      }
    })
    .catch((error) => {
      ...
      if (error.message) {
        if (
          ...
          errorMessage =
            'Votre session a expiré. Veuillez <a href="' +
            baseUrl +
            'auth/login">vous reconnecter</a>.';
        } else if (error.message.includes("JSON")) {
          errorMessage =
            "Erreur de communication avec le serveur. Votre session a peut-être expiré.";
        } else {
 ...
 */
function loadClientLocationsSimple(
  clientId,
  ...
  // Afficher un indicateur de chargement
  locationsContainer.innerHTML =
    '<div class="text-center"><i class="fas fa-spinner fa-spin"></i> Chargement des localisations...</div>';
  ...
  fetch(baseUrl + "user/load_client_locations", {
    ...
    .then((data) => {
      if (data && data.html) {
        ...
        locationsContainer.innerHTML =
          '<p class="text-danger">' +
          (data.error || "Erreur lors du chargement des localisations.") +
          "</p>";
      } else {
        locationsContainer.innerHTML =
          '<p class="text-muted">Aucune localisation disponible pour ce client.</p>';
      }
    })
    .catch((error) => {
      console.error("Erreur lors du chargement des localisations:", error);
 ...
 */
function loadContractRoomsSimple(
  clientId,
  roomsContainerId,
  contractId = null,
) {
  ...
  roomsContainer.innerHTML =
    '<div class="text-center"><i class="fas fa-spinner fa-spin"></i> Chargement des salles...</div>';
  ...
  fetch(baseUrl + "contracts/load_client_rooms", {
    ...
    .then((data) => {
      if (data && data.html) {
        ...
        roomsContainer.innerHTML =
          '<p class="text-danger">' +
          (data.error || "Erreur lors du chargement des salles.") +
          "</p>";
      } else {
        roomsContainer.innerHTML =
          '<p class="text-muted">Aucune salle disponible pour ce client.</p>';
      }
    })
    .catch((error) => {
      console.error("Erreur lors du chargement des salles:", error);
