...
(function () {
  ...
  function initContractTypeHandling() {
    ...
    const isticketcontractCheckbox =
      document.getElementById("isticketcontract");

    if (
      !contractTypeSelect ||
      !ticketsNumberInput ||
      !isticketcontractCheckbox
    ) {
      return;
    }
    ...
    function updateFieldsBasedOnContractType() {
      const selectedType = contractTypes.find(
        (type) => type.id == contractTypeSelect.value,
      );
      if (selectedType) {
        ...
        if (!isEditMode) {
          ...
          if (
            ticketsNumberInput.value !== selectedType.default_tickets.toString()
          ) {
            ticketsNumberInput.value = selectedType.default_tickets;
          }
        }
