...
<div class="container-fluid flex-grow-1 container-p-y">
        ...
        <div class="ms-auto p-2 bd-highlight">
            ...
            if (isset($_GET['return_to']) && isset($_GET['client_id'])) {
                ...
                if ($returnTo === 'client') {
                    ...
                    if ($activeTab) $returnUrl .= '?active_tab=' . $activeTab;
                    $returnText = 'Retour au client';
                }
            }
            ?>
            <a href="<?= $returnUrl ?>" class="btn btn-secondary me-2">
                <i class="bi bi-arrow-left me-1"></i> <?= $returnText ?>
            </a>
            ...
            <?php else: ?>
                <button type="button" class="btn btn-warning me-2" disabled
            ...
            <?php if ($isAdmin): ?>
                ...
                <button type="button" class="btn btn-outline-danger me-2"
                        data-bs-toggle="modal" data-bs-target="#deleteInterventionModal"
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="alert alert-danger mb-0">
                        <strong><?= h($deleteWarningTitle) ?></strong>
                        <div class="mt-2"><?= h($deleteWarningText) ?></div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
                    <form method="POST"
                          action="<?= BASE_URL ?>interventions/delete/<?= $intervention['id'] ?>"
                          class="d-inline">
                        <?= csrf_field() ?>
                ...
                <div class="col-md-3">
                    <div class="d-flex flex-column gap-2">
                        ...
                        <div>
                            ...
                            <p class="form-control-static mb-0"><?= h($intervention['building_name']) ?></p>
                        </div>
                        <?php endif; ?>
                ...
                <div class="col-md-3">
                    <div class="d-flex flex-column gap-2">
                        ...
                        <div>
                            ...
                            <p class="form-control-static mb-0"><?= formatDateFrench($intervention['closed_at']) ?></p>
                        </div>
                        <?php endif; ?>
                    </div>
                </div>
                        ...
                        <div class="comment mb-3 p-3 border rounded">
                            <div class="d-flex justify-content-between align-items-start mb-2">
                                ...
                                <div>
                                    ...
                                    <?php if ($comment['is_observation']): ?><span class="badge bg-warning">Observation</span><?php endif; ?>
                                    ...
                                    <form action="<?= BASE_URL ?>interventions/editComment/<?= $comment['id'] ?>" method="post">
                                        ...
                                        <div class="modal-body">
                                            <div class="mb-3">
                                                ...
                                                <textarea class="form-control" name="comment" rows="4" required><?= h($comment['comment']) ?></textarea>
                                            </div>
                                            <div class="form-check mb-2">
                                                ...
                                                <label class="form-check-label" for="vbc<?= $comment['id'] ?>">Visible par le client</label>
                                            </div>
                                            <div class="form-check mb-2">
                                                <input class="form-check-input" type="checkbox" name="is_solution"
                                                       id="sol<?= $comment['id'] ?>" <?= $comment['is_solution'] ? 'checked' : '' ?>>
                                                <label class="form-check-label" for="sol<?= $comment['id'] ?>">Marquer comme solution</label>
                                            </div>
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" name="is_observation"
                                                       id="obs<?= $comment['id'] ?>" <?= $comment['is_observation'] ? 'checked' : '' ?>>
                                                <label class="form-check-label" for="obs<?= $comment['id'] ?>">Marquer comme observation</label>
                                            </div>
                                        </div>
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
    ...
    <button type="button" class="btn btn-sm btn-outline-primary position-fixed bottom-0 end-0 m-3"
            ...
            data-bs-toggle="modal" data-bs-target="#mailHistoryModal"
                        ...
                        <div class="list-group list-group-flush">
                            ...
                            <div class="list-group-item px-0">
                                <small class="text-muted d-block ps-3">
                                    ...
                                    par <?= isset($entry['changed_by_name']) && $entry['changed_by_name'] !== null ? h($entry['changed_by_name']) : 'Utilisateur inconnu' ?>
                                </small>
                                <div class="mt-1 ps-3">
        ...
        <div class="modal-dialog modal-xl modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div id="mailHistoryEmpty" class="text-muted" style="display:none;">Aucun email dans l'historique.</div>
                    <div id="mailHistoryTableWrap" style="display:none;">
                        <div class="table-responsive">
                            <table class="table table-striped align-middle mb-0">
                                <thead>
                                    <tr><th style="width:170px;">Date</th><th>Titre</th><th>Destinataires</th></tr>
                                </thead>
...
<div class="modal fade" id="contractDetailsModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body"><div id="contractDetailsContent"><div class="text-center"><div class="spinner-border text-primary" role="status"></div><p class="mt-2">Chargement…</p></div></div></div>
...
<div class="modal fade" id="addAttachmentModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form action="<?= BASE_URL ?>interventions/addMultipleAttachments/<?= $intervention['id'] ?>"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
                <form method="POST"
                      action="<?= BASE_URL ?>interventions/reopen/<?= $intervention['id'] ?>"
                      class="d-inline">
                    <?= csrf_field() ?>
...
<!-- ══════════════════════════════════════════════════════════════════
     MODALE FERMETURE
     ══════════════════════════════════════════════════════════════════ -->
<div class="modal fade" id="modalFermerInter" tabindex="-1" aria-hidden="true"
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">
                    ...
                    <small class="text-muted ms-2"><?= h($intervention['reference'] ?? '') ?></small>
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            ...
            <form action="<?= BASE_URL ?>interventions/forceTickets/<?= $intervention['id'] ?>" method="POST">
                ...
                <div class="modal-body">
                    <div class="alert alert-warning">
                        ...
                        <strong>Attention !</strong> Cette action modifiera le nombre de tickets utilisés pour cette intervention fermée.
                    </div>
                    <div class="mb-3">
...
<div class="modal fade" id="pdfViewerModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-fullscreen">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="bi bi-file-pdf text-danger me-2"></i><span id="pdfFileName">Document PDF</span></h5>
                <div class="ms-auto me-3">
                    <button type="button" class="btn btn-sm btn-outline-secondary" id="pdfZoomOutBtn"><i class="bi bi-zoom-out"></i></button>
                    <button type="button" class="btn btn-sm btn-outline-secondary" id="pdfZoomInBtn"><i class="bi bi-zoom-in"></i></button>
                    <button type="button" class="btn btn-sm btn-outline-secondary" id="pdfResetZoomBtn"><i class="bi bi-arrow-repeat"></i></button>
                    <span id="pdfPageInfo" class="mx-2"></span>
                    <button type="button" class="btn btn-sm btn-outline-secondary" id="pdfPrevPageBtn"><i class="bi bi-chevron-left"></i></button>
                    <button type="button" class="btn btn-sm btn-outline-secondary" id="pdfNextPageBtn"><i class="bi bi-chevron-right"></i></button>
                </div>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            ...
            <div class="modal-footer">
                <a href="#" id="pdfDownloadLink" class="btn btn-primary" target="_blank"><i class="bi bi-download me-1"></i> Télécharger</a>
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Fermer</button>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="imageViewerModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-fullscreen">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="bi bi-image-fill text-primary me-2"></i><span id="imageFileName">Image</span></h5>
                <div class="ms-auto me-3">
                    <button type="button" class="btn btn-sm btn-outline-secondary" id="imageZoomOutBtn"><i class="bi bi-zoom-out"></i></button>
                    <button type="button" class="btn btn-sm btn-outline-secondary" id="imageZoomInBtn"><i class="bi bi-zoom-in"></i></button>
                    <button type="button" class="btn btn-sm btn-outline-secondary" id="imageResetZoomBtn"><i class="bi bi-arrow-repeat"></i></button>
                </div>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body p-0" style="background:#f5f5f5;">
                <div id="imageViewerContainer" style="height:calc(100vh - 120px);overflow:auto;text-align:center;">
                    <img id="viewerImage" src="" alt="" style="max-width:100%;height:auto;cursor:zoom-in;transition:transform 0.1s ease;">
                </div>
            </div>
            <div class="modal-footer">
                <a href="#" id="imageDownloadLink" class="btn btn-primary" target="_blank"><i class="bi bi-download me-1"></i> Télécharger</a>
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Fermer</button>
            </div>
        </div>
    </div>
...
<!-- ════════════════════════════════════════════════════════════════════════
     ...
     ════════════════════════════════════════════════════════════════════════ -->
<style>
.drop-zone{border:2px dashed var(--bs-border-color);border-radius:8px;padding:30px;text-align:center;background-color:var(--bs-body-bg);transition:all .3s ease;min-height:150px;display:flex;flex-direction:column;justify-content:center;align-items:center;}
.drop-zone.dragover{border-color:var(--bs-primary);background-color:var(--bs-primary-bg-subtle);}
.drop-message{font-size:1.1em;color:var(--bs-secondary-color);margin-bottom:15px;}
.drop-message i{font-size:2.5em;margin-bottom:10px;display:block;}
.file-list{margin-top:15px;max-height:200px;overflow-y:auto;}
.file-item{display:flex;align-items:center;padding:8px;margin:3px 0;border-radius:5px;border:1px solid var(--bs-border-color);background-color:var(--bs-body-bg);gap:10px;}
.file-item.valid{background-color:var(--bs-success-bg-subtle);border-color:var(--bs-success-border-subtle);}
.file-item.invalid{background-color:var(--bs-danger-bg-subtle);border-color:var(--bs-danger-border-subtle);}
.stats{margin-top:10px;padding:8px;background-color:var(--bs-secondary-bg);border-radius:5px;font-size:.9em;}
.progress-bar{height:3px;background-color:var(--bs-secondary-bg);border-radius:2px;overflow:hidden;margin-top:8px;}
.progress-fill{height:100%;background-color:var(--bs-primary);width:0%;transition:width .3s ease;}
.attachment-name{display:flex;flex-direction:column;}
.attachment-name .display-name{font-weight:500;color:var(--bs-body-color);}
.attachment-name .original-name{font-size:.75em;margin-top:2px;opacity:.7;font-style:italic;}
</style>
...
function loadContractDetails(contractId){
    var d=document.getElementById('contractDetailsContent');
    d.innerHTML='<div class="text-center"><div class="spinner-border text-primary" role="status"></div><p class="mt-2">Chargement…</p></div>';
    fetch('<?= BASE_URL ?>interventions/getContractInfo/'+contractId)
    .then(function(r){return r.json();})
    .then(function(data){
        if(data.error){d.innerHTML='<div class="alert alert-danger">'+data.error+'</div>';return;}
        var s=data.start_date?new Date(data.start_date).toLocaleDateString('fr-FR'):'Non définie';
        var e=data.end_date?new Date(data.end_date).toLocaleDateString('fr-FR'):'Non définie';
        var tc=data.isticketcontract==1;
        var tc_color=data.tickets_remaining>3?'success':(data.tickets_remaining>0?'warning':'danger');
        var tr_row=tc?'<tr><th class="text-muted">Tickets restants:</th><td><span class="badge bg-'+tc_color+'">'+( data.tickets_remaining||0)+'</span></td></tr>':'';
        d.innerHTML='<div class="row"><div class="col-md-6"><h6 class="fw-bold mb-3">Informations du contrat</h6><table class="table table-sm"><tr><th class="text-muted">Type:</th><td>'+(data.type_name||'Non défini')+'</td></tr><tr><th class="text-muted">Nature:</th><td>'+(tc?'<span class="badge bg-info">À tickets</span>':'<span class="badge bg-secondary">Sans tickets</span>')+'</td></tr><tr><th class="text-muted">Début:</th><td>'+s+'</td></tr><tr><th class="text-muted">Fin:</th><td>'+e+'</td></tr>'+tr_row+'</table></div></div>'+(data.comment?'<div class="row mt-3"><div class="col-12"><h6 class="fw-bold mb-2">Commentaire</h6><div class="alert alert-info">'+data.comment+'</div></div></div>':'');
    })
    .catch(function(){d.innerHTML='<div class="alert alert-danger">Erreur de chargement.</div>';});
}
</script>
...
(function(){
    ...
    /* Récupère le token CSRF depuis window.CSRF_TOKEN (injecté en haut de page)
       ou depuis le champ caché du formulaire, ou depuis la session PHP */
    ...
    function renderFermeture(data){
        ...
        html += '<thead class="table-light"><tr>'
            + '<th>Technicien</th><th>Durée</th><th>Dépl.</th>'
            + '<th>Qualifié</th><th>Détail calcul</th><th class="text-end">Tickets</th>'
            + '</tr></thead><tbody>';
...
function loadTechniciansInPage(){
    ...
    if(!container)return;
    var interventionId=<?= (int)($intervention['id']??0) ?>;
    var isFlash=<?= isset($intervention['is_flash'])&&$intervention['is_flash']==1?'true':'false' ?>;
    var needsCompletion=<?= isset($intervention['needs_completion'])&&$intervention['needs_completion']==1?'true':'false' ?>;
    var missingFields=<?php
        $missing=[];
        if(empty($intervention['site_id']))$missing[]='Site';
        if(empty($intervention['room_id']))$missing[]='Salle';
        if(empty($intervention['description']))$missing[]='Description';
        if(empty($intervention['demande_par']))$missing[]='Demandeur';
        if(empty($intervention['title'])||$intervention['title']=='')$missing[]='Titre';
    ...
    .then(function(data){
        ...
        assigned.forEach(function(tech){
            ...
            var st=tech.start_time?formatDateTime(tech.start_time):'Non défini';
            ...
            if(isFlash&&needsCompletion&&missingFields.length>0){
                ...
                missingFields.forEach(function(f){flashAlert+='<span class="badge bg-warning text-dark"><i class="bi bi-exclamation-circle"></i> '+f+'</span>';});
                flashAlert+='</div></div><div class="mt-2"><a href="'+window.BASE_URL+'interventions/edit/'+interventionId+'" class="btn btn-sm btn-warning w-100"><i class="bi bi-pencil-square"></i> Compléter</a></div></div>';
            }
            html+='<div class="list-group-item"><div class="d-flex justify-content-between align-items-start">'
                +'<div style="flex:1;"><strong><i class="bi bi-person-badge"></i> '+escapeHtml(name)+'</strong> '+qual+'<br>'
                +'<div class="row mt-2"><div class="col-md-6"><small class="text-muted"><i class="bi bi-calendar"></i> Début: '+st+'<br><i class="bi bi-calendar-check"></i> Fin: '+et+'</small></div>'
                +'<div class="col-md-6"><small class="text-muted"><i class="bi bi-clock"></i> Durée: '+tp+'<br><i class="bi bi-car"></i> Déplacement: '+dep+'</small></div></div>'
                ...
                +'<button class="btn btn-sm btn-outline-primary" onclick="sendEmailToTechnician('+tech.technicien_id+',\''+escapeHtml(name)+'\')" title="Email"><i class="bi bi-envelope"></i></button>'
                +'<button class="btn btn-sm btn-outline-danger" onclick="removeTechnicianFromPage('+tech.technicien_id+')" title="Retirer"><i class="bi bi-trash"></i></button>'
                +'</div></div></div>';
        });
        html+='</div>';
...
function removeTechnicianFromPage(technicianId){
    ...
    .then(function(data){
        ...
        return fetch(window.BASE_URL+'interventions/assignTechnicians',{method:'POST',headers:{'Content-Type':'application/json','X-Requested-With':'XMLHttpRequest'},credentials:'same-origin',body:JSON.stringify({intervention_id:parseInt(interventionId),technicians:toSave})});
    })
    .then(function(r){return r.json();})
    .then(function(result){
...
function displayRoundedTime(){
    ...
    if(v>0){
        ...
        var w=v!==r&&r>0?'<br><small class="text-primary">Sera arrondi à '+ft+' ('+r+' min)</small>':'';
        d.innerHTML='<i class="bi bi-calculator-fill text-primary"></i> <strong>Saisie :</strong> '+v+' min<br><strong>Après arrondi :</strong> '+ft+w;
...
function saveAllTechnicians(){
    ...
    fetch(window.BASE_URL+'interventions/assignTechnicians',{method:'POST',headers:{'Content-Type':'application/json','X-Requested-With':'XMLHttpRequest'},credentials:'same-origin',body:JSON.stringify({intervention_id:parseInt(interventionId),technicians:toSave})})
    .then(function(r){return r.json();})
    .then(function(result){
...
async function sendEmailToTechnician(technicianId,technicianName){
    ...
    if(btn){btn.innerHTML='<span class="spinner-border spinner-border-sm" role="status"></span>';btn.disabled=true;}
    try{
...
function formatDateTime(s){if(!s)return'';return new Date(s).toLocaleString('fr-FR',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'});}
...
/* Forcer tickets — calcul temps réel */
(function(){
    var inp=document.getElementById('new_tickets');
    if(!inp)return;
    var cur=parseFloat(inp.dataset.currentTickets)||0;
    var rem=parseFloat(inp.dataset.contractRemaining)||0;
    document.getElementById('current_remaining').textContent=rem;
    inp.addEventListener('input',function(){
        var nv=parseFloat(this.value)||0;
        var diff=nv-cur;
        var newRem=rem-diff;
        document.getElementById('new_remaining').textContent=newRem.toFixed(2)+(newRem<0?' ⚠️ (solde négatif)':'');
    });
