        ...
        <div class="ms-auto p-2 bd-highlight">
            ...
            <?php if (canManageContracts()): ?>
                ...
                <?php else: ?>
                    <span class="badge bg-secondary me-2"
                        title="Renouvellement disponible dans <?= $daysUntilEnd - 30 ?> jours">
            ...
            <?php if ($isAdmin && !empty($contract['contract_type_id'])): ?>
                ...
                <button type="button" class="btn btn-outline-danger btn-sm"
                    onclick="confirmDeleteContract(<?php echo $contract['id']; ?>, '<?php echo htmlspecialchars($contract['name'] ?? ''); ?>')"
    ...
    <div class="card mb-4">
        <div class="card-header py-2">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <span
                    class="badge bg-<?php echo $contract['status'] === 'actif' ? 'success' : ($contract['status'] === 'inactif' ? 'danger' : 'warning'); ?>">
                                ...
                                <td>
                                    ...
                                    <?php if ($isAdmin): ?>
                                        <button type="button" class="btn btn-sm btn-outline-primary ms-2" data-bs-toggle="modal"
                                            data-bs-target="#addTicketsModal" title="Ajouter des tickets">
                            ...
                            <td>
                                <?php if (isContractTicketById($contract['id'])): ?>
                                    ...
                                    <button type="button" class="btn btn-sm btn-outline-info ms-2" data-bs-toggle="modal"
                                        data-bs-target="#ticketsHistoryModal" title="Voir l'historique des tickets">
                        ...
                        <?php if (canManageContracts() && !isContractTicketById($contract['id'])): ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <br><small class="text-muted">Disponible uniquement pour les contrats sans
                                        tickets</small>
                                </td>
                            </tr>
                        <?php endif; ?>
                ...
                foreach ($attachments as $attachment):
                    ...
                    <div
                        ...
                        <div class="card-header py-1 d-flex justify-content-between align-items-center">
                            ...
                            <div>
                                <button type="button" class="btn btn-sm btn-outline-info btn-action" data-bs-toggle="modal"
                                    data-bs-target="#previewModal<?= $attachment['id'] ?>" title="Aperçu">
                        ...
                        <?php if (!empty($interventions)): ?>
                            <?php foreach ($interventions as $intervention): ?>
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge"
                                            style="background-color: <?= $intervention['status_color'] ?? '#6c757d' ?>">
...
<div class="modal fade" id="addAttachmentModal" tabindex="-1" aria-labelledby="addAttachmentModalLabel"
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form action="<?php echo BASE_URL; ?>contracts/addMultipleAttachments/<?php echo $contract['id']; ?>"
...
<!-- Le code d'initialisation est dans public/assets/js/pages/contracts.js -->
<!-- Code JavaScript supprimé - Utilise maintenant DragDropUploader.js et contracts.js -->

<!-- JavaScript spécifique à la page contracts -->
<script src="<?php echo BASE_URL; ?>assets/js/pages/contracts.js"
    onerror="console.error('ERREUR: contracts.js n\'a pas pu être chargé. Vérifiez que le fichier existe et est accessible.');"></script>
            ...
            <form action="<?php echo BASE_URL; ?>contracts/addTickets/<?php echo $contract['id']; ?>" method="post"
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control" id="add_tickets_date" name="add_tickets_date"
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="new_num_facture" name="new_num_facture"
                    ...
                    <div class="mb-3">
                        ...
                        <input type="file" class="form-control" id="avenant_file" name="avenant_file"
                    ...
                    <?php if ($shouldShowExtension): ?>
                        <div class="mb-3">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="extend_contract" name="extend_contract"
                                    value="1">
                                <label class="form-check-label" for="extend_contract">
                                    <i class="bi bi-calendar-plus me-1"></i>Prolonger le contrat jusqu'au 31 décembre de
                                    l'année suivante
                                </label>
                            </div>
                            <div class="form-text">
                                <small class="text-muted">
                                    <i class="bi bi-info-circle me-1"></i>
                                    Si le contrat se termine dans l'année courante, il sera prolongé jusqu'au 31 décembre de
                                    l'année suivante.
                                </small>
                            </div>
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="add_tickets_comment" name="add_tickets_comment" rows="3"
...
<button type="button" class="btn btn-sm btn-outline-secondary position-fixed bottom-0 end-0 m-3" data-bs-toggle="modal"
    data-bs-target="#historyModal" title="Historique des modifications">
            ...
            <form action="<?php echo BASE_URL; ?>contracts/renew/<?php echo $contract['id']; ?>" method="post">
                ...
                <div class="modal-body">
                        ...
                        <div class="col-md-6">
                            ...
                            <ul class="list-unstyled">
                                ...
                                <?php else: ?>
                                    ...
                                    <li><strong>Date de fin :</strong>
                                        <?= formatDateFrench(date('Y-m-d', strtotime($contract['end_date'] . ' +365 days'))) ?>
                                    </li>
                                <?php endif; ?>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="new_contract_name" name="new_contract_name"
                    ...
                    <?php if (isContractTicketById($contract['id'])): ?>
                        <div class="mb-3">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="reset_tickets" name="reset_tickets"
                                    value="1" checked>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="renewal_comment" name="renewal_comment" rows="3"
...
<!-- Le JavaScript est maintenant dans public/assets/js/pages/contracts.js -->
